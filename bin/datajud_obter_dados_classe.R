...
for (classe_codigo in classe_codigos){  
    message(sprintf("[INFO] Pesquisa iniciada para sobre %s ...",classe_codigo))
    query <- body_query_classe_codigo(
      classe_codigo = classe_codigo
    )  
    resultado_raw <- pesquisar_tribunal(tribunal, query)
    message("[INFO] Pesquisa concluída com sucesso.")
    
    # Salva como NDJSON (newline-delimited JSON)
    message("[INFO] Salvando NDJSON...")
    timestamp <- format(Sys.time(), "%Y%m%d%H%M%S")
    nome_arquivo <- glue("datajud_raw_{classe_codigo}_{timestamp}.ndjson")
    path_arquivo <- file.path(data_dir, nome_arquivo)

    # Extrair _source de cada hit e gravar linha por linha
    sources <- lapply(resultado_raw, function(x) x[["_source"]])
    writeLines(sapply(sources, jsonlite::toJSON, auto_unbox = TRUE), path_arquivo)
    message(glue("[INFO - FINAL] Arquivo salvo: {path_arquivo}\n"))
}
