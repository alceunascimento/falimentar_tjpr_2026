...
cran_pkgs <- c(
  ...
  "future",      # Para processamento paralelo
  "furrr"        # Extensão do purrr para operações paralelas usando future
)
...
n_cores <- min(length(classe_codigos), availableCores() - 1L)
...
plan(multisession, workers = min(length(classe_codigos), availableCores() - 1L))
...
plan(sequential)  # restaurar serializado ao final
